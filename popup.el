...
;; it feels like having something popping up the second you type is not fun
;; it makes you jump a little. but hooking into the idle callback and giving you
;; .5 seconds of time for help to popup works great. 
;; *************************************************************************
...
(defvar popupinfo-runtime-timer nil
  "the timer, if any is set.")
(make-variable-buffer-local 'popupinfo-runtime-timer)

(defun popupinfo-runtime-hook ()
  "dispatch the buffer cb"
  (if (and popupinfo-runtime-cb popupinfo-runtime-cb-enabled)
	  (progn
		(and popupinfo-runtime-timer (cancel-timer popupinfo-runtime-timer))
		(setq popupinfo-runtime-timer 
			  (run-with-idle-timer (or (current-idle-time) .5) nil popupinfo-runtime-cb)
			  ))))
...
(defun popupinfo-oncommand-set-cb (cb)
  ...
  (popupinfo-set-cb cb)
  (if popupinfo-runtime-timer
	  (progn 
		(cancel-timer popupinfo-runtime-timer)
		(setq popupinfo-runtime-timer nil)))
